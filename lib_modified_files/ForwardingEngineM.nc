...
/*
 * To be placed at tos/lib/net/tymo/
 */
...
implementation {
  ...
  command error_t AMSend.send[uint8_t am](am_addr_t addr, message_t * msg, uint8_t len){
    ...
    case FW_BROADCAST:                          //MODIFICATION
      call PPacket.setPayloadLength(msg, len);
      typebuf = am;                             //MODIFICATION
      dbg("fwe", "FE: Sending around - size: %hhu\n", len);
      return call SubSend.send(addr, msg, len); //MODIFICATION
  ...
  event message_t * SubReceive.receive(message_t * msg, void * payload, uint8_t len){
    if(call AMPacket.destination(msg) == AM_BROADCAST_ADDR) {                                          //MODIFICATION
      dbg("fwe", "FE: Received broacasted message. Signaling it to upper layer - size: %hhu\n", len);  //MODIFICATION
      payload = call PPacket.getPayload(msg, call PPacket.payloadLength(msg));
      return signal Receive.receive[typebuf](msg, payload, len);
    } else 
        dbg("fwe", "FE: Received a message from %u that should be sent to %u - size: %hhu\n", call AMPacket.source(msg), call AMPacket.destination(msg), len);
  ...
  event void SubSend.sendDone(message_t * msg, error_t e){
    ...
    if ((e == SUCCESS) && acks && (call AMPacket.destination(msg)) != AM_BROADCAST_ADDR) {
  ...
  event void Timer.fired(){
    ...
    case FW_SEND:
      dbg("fwe", "FE: I'm retrying to send my message. - size: %hhu\n", lenWaiting);
      if (addrWaiting) {
