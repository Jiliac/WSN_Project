...
implementation {
  ...
  command fw_action_t RouteSelect.selectRoute(message_t * msg, addr_t * destination, uint8_t * am_type){
    if (destination != NULL)
      dbg("mhe", "MHE: Somebody wants a route, let's see... - for %u\n", *destination);
    else
    ...
    } else if (destination != NULL && (*destination) == AM_BROADCAST_ADDR) {    // MODIF
      dbg("mhe", "MHE: I'm broadcasting around !\n");                           // MODIF 
      return FW_BROADCAST;                                                      // MODIF 
    } else { 
